...
// Required for testing adding positions
// - options must start in the future
// - positions can only be added when the option starts
// - therefore after creating the option, test script needs to wait a few seconds before adding positions
describe("OneZero Contract", function () {
    let OneZero: any;
    let oneZero: any;
    let OZ: any;
    let oz: any;
    ...
    beforeEach(async function () {
        ...
        [owner, addr1, addr2, addr3, dummyChainlinkKeeper] = await ethers.getSigners();
        const NAME = "One Zero";
        const SYMBOL = "OZ";
        ...
        oneZero = await OneZero.deploy(oz.getAddress(), storage.getAddress(), MINIMUM_DURATION);
    ...
    it("Should initialise the storage address correctly", async function () {
        expect(await oneZero.getStorageAddress()).to.equal(await storage.getAddress());
    });
    ...
    it("Should not allow non-owners to add admins", async function () {
        await expect(oneZero.connect(addr1).updateAdmin(addr2.address, true)).to.be.revertedWith("Only owner can call this function");
    });

    it("Should allow owner to add admins", async function () {
        await expect(oneZero.connect(owner).updateAdmin(addr1.address, true)).to.not.be.reverted;
    });
    ...
    // Storing and retrieving the OZ address accurately has been tested as part of checking if the OZ address has been initialised correctly

    // Storing and retrieving the storage address accurately has been tested as part of checking if the storage address has been initialised correctly

    // Storing and retrieving the owner accurately has been tested as part of checking if the owner has been initialised correctly

    it("Should not allow non-owners to transfer ownership", async function () {
        await expect(oneZero.connect(addr1).transferOwnership(addr2.address)).to.be.revertedWith("Only owner can call this function");
    });

    it("Should allow owner to transfer ownership", async function () {
        await expect(oneZero.connect(owner).transferOwnership(addr1.address)).to.not.be.reverted;
    });
    ...
    it("Should not allow non-owners to update the minimum duration", async function () {
        await expect(oneZero.connect(addr1).setMinimumDuration(MINIMUM_DURATION)).to.be.revertedWith("Only owner can call this function");
    });

    it("Should allow owner to update the minimum duration", async function () {
        await expect(oneZero.connect(owner).setMinimumDuration(MINIMUM_DURATION)).to.not.be.reverted;
    });
    ...
    it("Should retrieve the correct list of binary options a user has participated in", async function () {
        ...
        let userParticipatedOptions = await oneZero.getUserParticipatedOptions(addr1.address);
    ...
    it("Should retrieve the correct list of concluded binary options", async function () {
        ...
        await oneZero.performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0), BigInt(1)]]));
    ...
    it("Should retrieve the correct details of a user's long position", async function () {
        ...
        let longPosition = await oneZero.getUserLongPosition(BigInt(0), addr1.address);
        expect(longPosition).to.equal(BigInt(1e18 * (1 - Number(COMMISSION_RATE) / 10000)));
    });
    ...
    it("Should retrieve the correct details of a user's short position", async function () {
        ...
        let shortPosition = await oneZero.getUserShortPosition(BigInt(0), addr1.address);
        expect(shortPosition).to.equal(BigInt(1e18 * (1 - Number(COMMISSION_RATE) / 10000)));
    });
    ...
    it("Should not allow non-owner and non-admins to add binary options", async function () {
        ...
        const START: bigint = BigInt(latestBlock.timestamp) + BigInt(10);
        const COMMISSION_RATE: bigint = BigInt(10);
        await expect(oneZero.connect(addr1).addBinaryOption(TITLE, START, DURATION, COMMISSION_RATE)).to.be.revertedWith("Only owner and admins can call this function");
    });
    ...
    it("Should allow owner to add binary options", async function () {
        ...
        const START: bigint = BigInt(latestBlock.timestamp) + BigInt(10);
        const COMMISSION_RATE: bigint = BigInt(10);
        await expect(oneZero.connect(owner).addBinaryOption(TITLE, START, DURATION, COMMISSION_RATE)).to.not.be.reverted;
    });
    ...
    it("Should allow admins to add binary options", async function () {
        ...
        const COMMISSION_RATE: bigint = BigInt(10);
        await oneZero.connect(owner).updateAdmin(addr1.address, true);
        await expect(oneZero.connect(addr1).addBinaryOption(TITLE, START, DURATION, COMMISSION_RATE)).to.not.be.reverted;
    });
    ...
    it("Should return false when chainlink keeper calls checkUpkeep and there are no options past their expiry", async function () {
        ...
        let checkUpkeepFirstResponse = await oneZero.connect(dummyChainlinkKeeper).checkUpkeep(ethers.AbiCoder.defaultAbiCoder().encode([], []));
        expect(checkUpkeepFirstResponse[0]).to.be.false;
        expect(checkUpkeepFirstResponse[1]).to.equal(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[]]));
        await network.provider.send("evm_increaseTime", [20]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for option to start
        // After option starts, checkUpkeep should also return false
        let checkUpkeepSecondResponse = await oneZero.connect(dummyChainlinkKeeper).checkUpkeep(ethers.AbiCoder.defaultAbiCoder().encode([], []));
        expect(checkUpkeepSecondResponse[0]).to.be.false;
        expect(checkUpkeepSecondResponse[1]).to.equal(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[]]));
    });
    ...
    it("Should return true when chainlink keeper calls checkUpkeep and there are options past their expiry", async function () {
        ...
        const COMMISSION_RATE: bigint = BigInt(10);
        await oneZero.addBinaryOption(TITLE, START, DURATION, COMMISSION_RATE);
        await oneZero.addBinaryOption(TITLE, START, DURATION + BigInt(500), COMMISSION_RATE);
        await network.provider.send("evm_increaseTime", [1220]);
        ...
        expect(checkUpkeepFirstResponse[1]).to.equal(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]));
        ...
        expect(checkUpkeepSecondResponse[1]).to.equal(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0), BigInt(1)]]));
    });
    ...
    it("Should not allow performUpkeep to conclude options prematurely", async function () {
        ...
        await expect(oneZero.connect(addr1).performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]))).to.be.revertedWith("Duration for binary option has not passed");
    });
    ...
    it("Should not throw any errors if binary option concludes without any stakers", async function () {
        ...
        await network.provider.send("evm_increaseTime", [1220]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for first option to expire
        await expect(oneZero.connect(dummyChainlinkKeeper).performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]))).to.not.be.reverted;
    });
    ...
    it("Should not allow performUpkeep to conclude an option multiple times", async function () {
        ...
        await network.provider.send("evm_increaseTime", [1220]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for option to expire
        await oneZero.performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]));
        ...
        await expect(oneZero.connect(addr1).performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]))).to.be.revertedWith("Binary option has already been concluded");
    });
    ...
    it("Should pay out commissions correctly when an option is concluded and there are winners (single token holder)", async function () {
        ...
        await network.provider.send("evm_increaseTime", [20]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for option to start
        await oneZero.connect(addr3).addPosition(BigInt(0), true, {value: BigInt(3e18)}); // addr3 goes long with 3 ether, total commission = 0.003 ether
        await network.provider.send("evm_increaseTime", [1220]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for option to expire
        let ownerBalanceBeforeCommission = await ethers.provider.getBalance(owner.address);
        let addr1BalanceBeforeCommission = await ethers.provider.getBalance(addr1.address);
        await oneZero.connect(dummyChainlinkKeeper).performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]));
        expect (await ethers.provider.getBalance(owner.address)).to.equal(ownerBalanceBeforeCommission + BigInt(0.003 * 1e18)); // owner should receive 0.003 ether commission
        expect (await ethers.provider.getBalance(addr1.address)).to.equal(addr1BalanceBeforeCommission); // addr1 should not have received any ether commission
    });
    ...
    it("Should pay out commissions correctly when an option is concluded and there are no winners (single token holder)", async function () {
        ...
        await network.provider.send("evm_increaseTime", [20]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for option to start
        await oneZero.connect(addr3).addPosition(BigInt(0), false, {value: BigInt(3e18)}); // addr3 goes long with 3 ether, total commission = 0.003 ether
        await network.provider.send("evm_increaseTime", [1220]);
        await network.provider.send("evm_mine"); // Mine a new block to skip ahead for option to expire
        let ownerBalanceBeforeCommission = await ethers.provider.getBalance(owner.address);
        let addr1BalanceBeforeCommission = await ethers.provider.getBalance(addr1.address);
    ...
    it("Should pay out commissions correctly when an option is concluded and there are winners (multiple token holders)", async function () {
        ...
        let ownerBalanceBeforeCommission = await ethers.provider.getBalance(owner.address);
        let addr1BalanceBeforeCommission = await ethers.provider.getBalance(addr1.address);
        let addr2BalanceBeforeCommission = await ethers.provider.getBalance(addr2.address);
        await oneZero.connect(dummyChainlinkKeeper).performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]));
        expect (await ethers.provider.getBalance(owner.address)).to.equal(ownerBalanceBeforeCommission + BigInt(0.002 * 1e18)); // owner should receive 0.002 ether commission
        expect (await ethers.provider.getBalance(addr1.address)).to.equal(addr1BalanceBeforeCommission + BigInt(0.001 * 1e18)); // addr1 should receive 0.001 ether commission
        expect (await ethers.provider.getBalance(addr2.address)).to.equal(addr2BalanceBeforeCommission); // addr2 should not have received any ether commission
    });
    ...
    it("Should pay out winnings correctly when an option is concluded (no oz token holders, longs win, multiple winners, single loser)", async function () {
        ...
        expect (await ethers.provider.getBalance(addr1.address)).to.equal(addr1BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * (BigInt(10000) - COMMISSION_RATE) / BigInt(10000) * BigInt(1) / BigInt(3)); // Longs won so addr1 should receive 1/3 of the total longs and shorts (less commission)
        expect (await ethers.provider.getBalance(addr2.address)).to.equal(addr2BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * (BigInt(10000) - COMMISSION_RATE) / BigInt(10000) * BigInt(2) / BigInt(3)); // Longs won so addr2 should receive 2/3 of the total longs and shorts (less commission)
        expect (await ethers.provider.getBalance(addr3.address)).to.equal(addr3BalanceBeforeWinnings); // Shorts lost so addr3 should not receive anything
    });
    ...
    it("Should pay out winnings correctly when an option is concluded (no oz token holders, longs win, single winner, multiple losers)", async function () {
        ...
        expect (await ethers.provider.getBalance(addr3.address)).to.equal(addr3BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * (BigInt(10000) - COMMISSION_RATE) / BigInt(10000)); // Longs won so addr3 will receive the full amount (less commission) since only addr3 won
    });
    ...
    it("Should pay out winnings correctly when an option is concluded (multiple oz token holders, longs win, multiple winners, single loser)", async function () {
        ...
        expect (await ethers.provider.getBalance(addr1.address)).to.equal(addr1BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * (BigInt(10000) - COMMISSION_RATE) / BigInt(10000) * BigInt(1) / BigInt(3) + BigInt(1e18 + 2e18 + 3e18) * COMMISSION_RATE / BigInt(10000) * BigInt(1000) / BigInt(4000)); // Longs won so addr1 should receive 1/3 of the total longs and shorts (less commission) and 1/4 of 0.006 ether commission
        expect (await ethers.provider.getBalance(addr2.address)).to.equal(addr2BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * (BigInt(10000) - COMMISSION_RATE) / BigInt(10000) * BigInt(2) / BigInt(3)); // Longs won so addr2 should receive 2/3 of the total longs and shorts (less commission) but no commission since addr2 does not hold any oz tokens
        expect (await ethers.provider.getBalance(addr3.address)).to.equal(addr3BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * COMMISSION_RATE / BigInt(10000) * BigInt(1000) / BigInt(4000)); // Shorts lost so addr3 should not receive winnings but will receive 1/4 of 0.006 ether commission
    });
    ...
    it("Should pay out winnings correctly when an option is concluded (multiple oz token holders, longs win, single winner, multiple losers)", async function () {
        ...
        await oneZero.connect(dummyChainlinkKeeper).performUpkeep(ethers.AbiCoder.defaultAbiCoder().encode(["uint256[]"], [[BigInt(0)]]));
        expect (await ethers.provider.getBalance(owner.address)).to.equal(ownerBalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * COMMISSION_RATE / BigInt(10000) * BigInt(2000) / BigInt(4000)); // Owner does not receive any winnings but will receive commission of 1/2 of 0.006 ether commission
        expect (await ethers.provider.getBalance(addr1.address)).to.equal(addr1BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * COMMISSION_RATE / BigInt(10000) * BigInt(1000) / BigInt(4000)); // Shorts lost so addr1 should not receive winnings but will receive 1/4 of 0.006 ether commission
        expect (await ethers.provider.getBalance(addr2.address)).to.equal(addr2BalanceBeforeWinnings); // Shorts lost so addr2 should not receive winnings and will also not receive any commissions since addr2 does not hold any oz tokens
        expect (await ethers.provider.getBalance(addr3.address)).to.equal(addr3BalanceBeforeWinnings + BigInt(1e18 + 2e18 + 3e18) * (BigInt(10000) - COMMISSION_RATE) / BigInt(10000) + BigInt(1e18 + 2e18 + 3e18) * COMMISSION_RATE / BigInt(10000) * BigInt(1000) / BigInt(4000)); // Longs won so addr3 will receive the full amount (less commission) since only addr3 won and will also receive 1/4 of 0.006 ether commission
    });
